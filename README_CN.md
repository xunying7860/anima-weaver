...
| **Anima加载图像** | Anima Weaver | 从文件夹加载图片列表 → IMAGE list + 遮罩 + 文件路径 |
| **Anima列表转多行** | Anima Weaver | 将批次文本项合并为多行 STRING（INPUT_IS_LIST） |
...
| 模型 | 下拉 | LM Studio 模型选择（选 `none` 禁用 LLM，透传变化文本） |
| 生成后卸载 | 开关 | 生成后卸载模型 |
...
**none 模式（禁用 LLM）：** 模型下拉选择 `none` 后完全跳过 LLM 调用——变化文本（如 wd14 标签器的多行标签）原样透传，与固定前缀组合后直接输出/保存 txt。典型工作流：`Anima加载图像 → wd14 标签器 → Anima列表转多行 → 变化文本 → Anima反推(none) → 保存为txt`。

**提示词优先级：** 自定义系统提示词 > 有图→图像反推 > 无图→润色扩写

**批量模式：** 接 `种子串`（位于「随机种子」下方）后每种子独立生成描述（不接图像），LLM 加载一次，N 组并发执行，跑完后按需卸载。
...
| 变化文本 | 文本 (forceInput) | 可变文本，每行对应一张图片，与固定前缀和反推描述组合后保存到 txt |
| 图片路径 | 文本 | 批处理图片文件夹路径（优先级低于种子串；可右键转换为输入，接入 Anima加载图像 的文件路径串） |
...
#### 手动模式
- 保留原有全部功能，从 13 种比例下拉选择或随机

#### 自动模式（评分制决策引擎 v7）
- 输入提示词后，通过 **7 层优先级归一化 + 三维评分制决策引擎** 自动分析构图，输出最合适的比例
- 无需手动维护规则库，算法自动处理构图、人数、姿态、视角等维度

**支持 13 种比例：** 1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9

**评分机制（宽屏/竖屏/方屏三维评分）：**
- **宽屏因子：** 战斗(+3)、动作(+2)、过肩镜头(+3)、广角(+2~3)、航拍(+2~4)、小群体(+1)、大群体(+5)、多人数(+2)、全景(+5)、全身+多人(+2)、对抗(+1)、躺卧(+2,覆盖坐姿)、拥抱+群像(+2)、仰拍(+1)
- **竖屏因子：** 竖长构图(+4~1)、自拍(+4)、全身+单人(+2)、半身/胸像(+1)、上半身/下半身(+2)、坐姿(+3)、蜷缩(+1)
- **方屏因子：** 面部特写(+5)、焦点细节(+5)、Q版(+4)、美式镜头(+3)、静物(+2)、俯拍(+1)
- **组合加分：** fighting+action(+1)、panorama+wide(+1)、full body+action(+1)
...
**7 层优先级归一化：**
- Layer 0: 复合词优先（避免 face close up 被 close up 污染）
- Layer 1: 人数与群体
- Layer 2: 景别与镜头
- Layer 3: POV 视角
- Layer 4: 动作与姿态
...
**输出变更：**
- 移除 分辨率、分辨率串、宽度串、高度串 输出
- 宽度/高度 改为 STRING 类型（手动=单值，自动=多行逗号分隔）
- 最终输出：随机种子(INT) + 种子串(STRING) + 宽度(STRING) + 高度(STRING)

**分辨率计算：**
- 全局最小边 512px，最大边 2048px
- 对齐步长 16（适配 Illustrious / anima-base DiT）
- 面积损失 < 3%

### 批量管线相关变更
- 同步串行：7路→6路（移除分辨率串通道）
- 串行拆分：7路→6路（移除分辨率通道）
- Anima随机提示词：移除分辨率输入、分辨率串输出
- Anima反推：移除分辨率输入、分辨率串输出
