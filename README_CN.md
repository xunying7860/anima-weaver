...
**新增功能：**
- 新增「模式」开关（BOOLEAN，默认 False=手动）
- 手动模式保留原有全部功能不变
- 自动模式（True）新增：提示词输入（STRING forceInput 多行）、纯随机分辨率（BOOLEAN 默认 True）
- 输入提示词时，按行分析关键词匹配构图类型（70%首选比例/30%次选比例）
- 提示词未接或行数不足时，由纯随机分辨率开关决定补齐方式（True=随机，False=固定）
- 支持 11 种比例：1:1, 3:4, 4:3, 2:3, 3:2, 16:9, 9:16, 16:10, 10:16, 21:9, 9:21
- 手动模式固定比例下拉补全至 11 种
...
**注意：自动模式算法基于关键词匹配，仅供参考（待验证）。欢迎提出更好的算法建议。**
