...
> ⚠️ **注意：** 如果使用带有思考链（推理）的模型（如 huihui-qwythos 系列），节点会自动在 API 请求中设置 `enable_thinking: false` 来关闭思考链，无需手动配置。非推理模型（如 `uncensored-ministral3-3b`、`llama-3.1-8b-lexi-uncensored_v2`）不受影响。
