        ...
        return {
            "required": {
                ...
                "自动上下文长度": (
                    "BOOLEAN",
                    {"default": False,
                     "tooltip": "启用后自动计算上下文长度 = 最大并发数 × 2048，并强制缩放图片到约 100 万像素，仅本地模型有效"},
                ),
                "最大截断长度": (
                    "INT",
        ...
        if _api_key:
            ...
            if _lm_model and _lm_model != "(no models found)":
                if not _preloaded:
                    ...
                    ensure_model_loaded(_lm_model, context_length=ctx, parallel=int(kwargs.get("最大并发数", 4)) if bool(kwargs.get("自动上下文长度", False)) else None)
    ...
    def _generate_one(self, kwargs: dict, user_msg: str, system_prompt: str) -> str:
        ...
        if api_key:
            ...
            if lm_model and lm_model != "(no models found)":
                ...
                from .lm_studio import ensure_model_loaded
                ensure_model_loaded(lm_model, context_length=ctx, parallel=int(kwargs.get("最大并发数", 4)) if bool(kwargs.get("自动上下文长度", False)) else None)
                nl = generate_nl_from_lm_studio(
                    user_msg, base_url,
                    model_name=lm_model,
                    detailed=True, timeout=180,
        ...
        if seed_str:
            ...
            if not _api_key and _lm_model and _lm_model != "(no models found)":
                try:
                    from .lm_studio import ensure_model_loaded
                    ctx = int(kwargs.get("上下文长度", 4096))
                    if ensure_model_loaded(_lm_model, context_length=ctx, parallel=int(kwargs.get("最大并发数", 4)) if bool(kwargs.get("自动上下文长度", False)) else None):
            ...
            if api_key:
                ...
                if lm_model and lm_model != "(no models found)":
                    if not kwargs.get("_preloaded"):
                        ctx = int(kwargs.get("上下文长度", 4096))
                        model_was_loaded = ensure_model_loaded(lm_model, context_length=ctx, parallel=int(kwargs.get("最大并发数", 4)) if bool(kwargs.get("自动上下文长度", False)) else None)
                    else:
