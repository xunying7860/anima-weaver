...
> ⚠️ **注意：** 如果使用带有思考链（推理）的模型（如 huihui-qwythos 系列），请在 LM Studio 中关闭思考链以加快生成速度。关闭方法：加载模型后，在 LM Studio 的模型配置中将 `thinking` 设为 `0` 或取消勾选「启用思考链」。非推理模型（如 `uncensored-ministral3-3b`、`llama-3.1-8b-lexi-uncensored_v2`）无需此设置。
